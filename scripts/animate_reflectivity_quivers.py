...
def load_frame(path_a, path_b, level, refl_name, u_name, v_name):
    """Load one frame's data.

    Returns x/y coordinate arrays (km), the z height (m), the combined
    reflectivity (NaN where invalid) and the u/v wind components — all as
    2-D NumPy arrays for the requested vertical ``level``.
    """
    ...
    try:
        ...
        return dict(x_km=x_km, y_km=y_km, z_m=z_m, refl=refl, u=u, v=v)
    finally:
        ds_a.close()
        ds_b.close()
...
def build_animation(timestamps, files_a, files_b, level, refl_name, u_name,
                    v_name, vmin, vmax, cmap, quiver_spacing_km, quiver_scale,
